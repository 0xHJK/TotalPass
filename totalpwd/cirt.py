...
class CirtPass(object):
    ...
    def update(cls):
        """
            更新密码
        """
        cirt_path = path.join(opts.pwds_path, "cirt")
        cls.mkdir(cirt_path)
        # 获取不同厂商的密码链接
        ...
        for i in range(opts.threads):
            t = Thread(target=cls.fetchpwds, args=(cirt_path,))
            t.start()
    ...
    @classmethod
    def fetchpwds(cls, cirt_path):
        """
            从URL获得数据并分析得到pwd保存到文件
        """
            ...
            try:
                ...
                for tb in d("table"):
                    ...
                    for cat in categories:
                        ...
                        pwd = pwds_map.get(new_table["title"], None)
                        cred = dict(
                        ...
                        if pwd:
                            if pwd.category == cat:
                                # 如果pwd已存在并且分类相同则只添加密码
                                pwd.credentials.append(cred)
                                continue
                            else:
                                # 如果分类不同也需要创建新的
                                title = model + " - " + cat
                ...
                # 保存获取到的数据
                for title, pwd in pwds_map.items():
                    fname = re.sub("[~`!@#\$%\^&\*\/\?\:\|]", "-", title)
                ...
                click.secho("[x] fetch pwds %s failed." % url, fg="red")
                cls.logger.error("%s Exception: %s" % (type(e).__name__, str(e)))
